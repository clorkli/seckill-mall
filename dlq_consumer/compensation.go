...
func markOrderFailedOrRetry(ctx context.Context, msg OrderMessage, reason string, d amqp.Delivery, span trace.Span) bool {
	if err := markOrderFailed(ctx, msg, reason); err != nil {
		log.Printf("dlq order mark_failed_status_failed order_id=%s err=%v", msg.OrderID, err)
...
func handleRollbackResult(ctx context.Context, code int, msg OrderMessage, d amqp.Delivery, span trace.Span) {
	switch code {
	case 0:
		log.Printf("dlq redis stock_key_missing order_id=%s product_id=%d", msg.OrderID, msg.ProductID)
	...
	case 1:
		...
		log.Printf("dlq compensation succeeded order_id=%s user_id=%d product_id=%d count=%d", msg.OrderID, msg.UserID, msg.ProductID, msg.Count)
	...
	case 2:
		...
		log.Printf("dlq compensation invalid_count order_id=%s count=%d", msg.OrderID, msg.Count)
	...
	case 3:
		...
		log.Printf("dlq compensation already_rolled_back order_id=%s", msg.OrderID)
	...
	case 4:
		...
		log.Printf("dlq compensation no_purchase_record order_id=%s", msg.OrderID)
	...
	case 5:
		...
		log.Printf("dlq compensation manual_check_required order_id=%s user_id=%d count=%d", msg.OrderID, msg.UserID, msg.Count)
		...
		d.Ack(false)
	default:
		log.Printf("dlq compensation unknown_rollback_status order_id=%s code=%d", msg.OrderID, code)
